...
<template>
    <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        <button
            ...
            :class="flipped.has(i)
                ? 'border-sky-300 bg-sky-600 text-on-dark'
                : 'border-paper-200 bg-card-fixed hover:border-sky-200'"
            @click="toggle(i)"
        >
            <span
                class="eyebrow mb-3 block"
                :class="flipped.has(i) ? 'text-on-dark-soft' : 'text-clay-500'"
            >
                {{ flipped.has(i) ? 'La realidad' : 'El mito' }}
            </span>
            <p
                class="grow font-serif text-lg leading-snug"
                :class="flipped.has(i) ? 'text-on-dark' : 'text-on-card-fixed'"
            >
                {{ flipped.has(i) ? item.truth : item.myth }}
            </p>
            <span
                class="mt-4 self-end text-xs font-bold"
                :class="flipped.has(i) ? 'text-on-dark-soft/80' : 'text-sky-400'"
            >
                Tocar para girar
            </span>
        </button>
