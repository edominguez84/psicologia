        ...
        <table class="w-full min-w-[640px] text-left text-sm">
            <thead class="border-b border-paper-200 text-xs font-bold uppercase tracking-wider text-sky-500">
                <tr>
                    ...
                    <th class="px-4 py-3">Nombre</th>
                    <th class="px-4 py-3">Contacto</th>
                    <th class="px-4 py-3">Conversación</th>
                    <th class="px-4 py-3">Estado</th>
                    <th class="px-4 py-3"></th>
                </tr>
            </thead>
            <tbody class="divide-y divide-paper-100">
                @forelse ($chatbotLeads as $lead)
                    <tr>
                        <td class="whitespace-nowrap px-4 py-3 align-top">{{ $lead->created_at->format('d/m/Y H:i') }}</td>
                        <td class="px-4 py-3 align-top font-semibold text-ink">{{ $lead->name }}</td>
                        <td class="px-4 py-3 align-top">
                            <p>{{ $lead->email }}</p>
                            <p class="text-xs text-ink-soft">{{ $lead->phone ?: '—' }}</p>
                        </td>
                        <td class="max-w-xs px-4 py-3 align-top text-ink-soft">
                            @php $lastUserMsg = collect($lead->transcript)->last(fn ($m) => ($m['from'] ?? null) === 'user' && ! in_array($m['text'] ?? '', [$lead->name, $lead->email, $lead->phone], true)); @endphp
                            {{ $lastUserMsg['text'] ?? \Illuminate\Support\Str::limit(collect($lead->transcript)->pluck('text')->implode(' · '), 120) }}
                        </td>
                        <td class="px-4 py-3 align-top">
                            @if ($lead->handled_at)
                                <span class="rounded-full bg-sky-100 px-2.5 py-1 text-xs font-semibold text-sky-700">Atendido</span>
                        ...
                        <td class="px-4 py-3 align-top">
                            <form method="POST" action="{{ route('admin.messages.chatbot.handle', $lead) }}">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="text-xs font-semibold text-sky-700 hover:underline">
                                    {{ $lead->handled_at ? 'Marcar pendiente' : 'Marcar atendido' }}
                                </button>
                            </form>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-4 py-6 text-center text-ink-soft">Todavía no hay contactos del chatbot.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    <div class="mt-3">{{ $chatbotLeads->links() }}</div>
</x-admin-layout>
