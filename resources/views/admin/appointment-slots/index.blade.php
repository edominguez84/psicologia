<x-admin-layout title="Horarios de citas">
    <h1 class="text-2xl font-serif text-sky-800">Horarios de citas</h1>
    <p class="mt-2 text-sm text-ink-soft">
        Crea las franjas horarias en las que puedes atender. Los pacientes solo ven (y pueden
        elegir) los horarios activos y todavía libres.
    </p>
            ...
            <div class="flex items-center justify-between gap-4 rounded-2xl border border-paper-200 bg-white p-4" @class(['opacity-50' => ! $slot->is_active])>
                <div>
                    ...
                    @if ($slot->active_appointments_count > 0)
                        <p class="text-xs font-semibold text-clay-500">Con una solicitud en curso</p>
                ...
                <div class="flex shrink-0 items-center gap-3 text-xs">
                    <form method="POST" action="{{ route('admin.appointment-slots.toggle', $slot) }}">
                        @csrf
                        @method('PATCH')
                    ...
                    <form method="POST" action="{{ route('admin.appointment-slots.destroy', $slot) }}" onsubmit="return confirm('¿Eliminar este horario definitivamente?')">
                        @csrf
                        @method('DELETE')
    ...
    <form method="POST" action="{{ route('admin.appointment-slots.store') }}" class="max-w-lg space-y-4">
        @csrf
        <div>
